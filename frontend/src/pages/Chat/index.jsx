...
const Chat = () => {
  ...
  const [conversationNameEditingKey, setConversationNameEditingKey] = useState(null);
  const isConversationNameEditing = record => record.id === conversationNameEditingKey;
  ...
  const setDefaultEntrypoint = defaultEntrypoint => {
    fetch(`${apiBaseUrl}/user/${user.id}/default-entrypoint`, {
      method: 'PATCH',
      headers: {
      ...
      .then(response => {
        if (response.success) {
          // message.success('Default entrypoint set to Home.');
          localStorage.setItem('user', JSON.stringify({ ...user, default_entrypoint: 'home' }));
        } else {
          handleTokenError(response.code);
        }
      })
  ...
  const getConversations = useCallback(() => {
    fetch(`${apiBaseUrl}/dify/conversations?limit=20`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
  ...
  const getMoreConversations = () => {
    fetch(`${apiBaseUrl}/dify/conversations?limit=20${lastId ? `&last_id=${lastId}` : ''}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
  ...
  const deleteConversation = conversationId => {
    fetch(`${apiBaseUrl}/dify/conversations/${conversationId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${userToken}`,
        'Content-Type': 'application/json',
  ...
  const renameConversation = (conversationId, newName) => {
    if (newName === '') {
      ...
      message.error('Conversation name is too long.');
      return;
    } else if (newName === conversations.find(item => item.id === conversationId).name) {
      // message.info('Conversation name unchanged.');
      return;
    }
    fetch(`${apiBaseUrl}/dify/conversations/${conversationId}/name`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${userToken}`,
      ...
      .catch(error => {
        message.error('Error renaming conversation.');
      });
  };
  ...
  const fetchConversationMessages = conversationId => {
    ...
    fetch(`${apiBaseUrl}/dify/messages?conversation_id=${conversationId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
  ...
  const getSuggestedQuestions = useCallback(message_id => {
    ...
    fetch(`${apiBaseUrl}/dify/messages/${message_id}/suggested`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
  ...
  const handleConversationClick = conversationId => {
    ...
    if (isMobile && !conversationNameEditingKey) setDrawerOpen(false); // 移动端点击后关闭抽屉
    if (conversationId !== currentConversation.conversation_id) {
      setCurrentConversation({
      ...
      items.unshift(
        {
          ...
          onClick: e => {
            ...
            const shareUrl = `${window.location.origin}/chat/share?cid=${conversationId}&uid=${user.uuid}`;
        ...
        },
        {
          ...
          onClick: e => {
            e.domEvent.stopPropagation();
            setConversationNameEditingKey(conversationId);
          },
        },
        {
          key: 'delete',
                ...
                ...conversations.map((item, index) => ({
                  ...
                  label: (
                    <div
                      ...
                      title={item.name}
                    >
                      {isConversationNameEditing(item) ? (
                        <input
                          type="text"
                          defaultValue={item.name}
                          onBlur={e => {
                            renameConversation(item.id, e.target.value);
                            setConversationNameEditingKey(null);
                          }}
                          onKeyDown={e => {
                            if (e.key === 'Enter') {
                              renameConversation(item.id, e.target.value);
                        ...
                        <span
                          ...
                          onDoubleClick={() =>
                            item.id !== '' && setConversationNameEditingKey(item.id)
                          }
                        >
                          {item.name}
                        </span>
        ...
        <Content>
          <ProChat
            ...
            request={async messages => {
              ...
              try {
                const response = await fetch(`${apiBaseUrl}/dify/chat-messages`, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${userToken}`,
